...
export default function ControlledForm() {
    ...
    const handleClick = (item) => {
        ...
        axios({
            url: "http://127.0.0.1:8000/api/users/"+item.id,
            method: "get",
            headers: {
                "accept": "application/json",
                "Authorization": "Bearer " + cookies.autif
            ...
            navigate("/changeUser", { state: data })
        });
    };

    const DeleteParams = (param_id) => {
        axios({
            url: "http://127.0.0.1:8000/api/users/" + param_id + "/",
            method: "delete",
            headers: {
                "Content-type": "application/json",
                "Authorization": "Bearer " + cookies.autif
    ...
    return (
        <div>
            ...
            <h2 style={{ textAlign: 'center' }}>Пользователи
                <AddCircleIcon onClick={() => { navigate("/addUser"); }} style={{ marginLeft: "20px" }} />
            </h2>
            <List sx={{ width: '80%', maxWidth: 800, bgcolor: 'background.paper', margin: "20px auto" }}>
                {categories?.map(item =>
                    <ListItem>
                        <ListItemText key={item.id} primary={item.username} secondary={item.role} />
                        ...
                        <span onClick={() => { handleClick(item); }} style={{ "position": "absolute", "top": "5px", "right": "40px", "background": "white", "width": "30px", "textAlign": "center", "borderRadius": "5px" }}><EditIcon /></span>
                    </ListItem>
                )}
            </List>
        </div>
